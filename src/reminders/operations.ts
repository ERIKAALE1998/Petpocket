...
export const getUserReminders: GetUserReminders = async (_args, context) => {
  if (!context.user) {
...
export const getBusinessUpcomingReminders: GetBusinessUpcomingReminders = async (_args, context) => {
...
type TriggerManualReminderOutput = {
  success: boolean;
  sentAt: Date | null;
};

export const triggerManualReminder: TriggerManualReminder<TriggerManualReminderInput, TriggerManualReminderOutput> = async (args, context) => {
  ensureUserRole(context.user, ["VET_BUSINESS", "ADMIN"]);
